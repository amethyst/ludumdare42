        ...
        if self.beat_points.is_none() {
            ...
            for (mut transform,_) in (&mut transforms, &players).join() {
                ...
                if self.last_beatpoint.as_ref() != self.beat_points.as_ref().unwrap().front() {
                    ...
                    let dir = self.beat_points.as_ref().unwrap().front().unwrap().0 - self.last_beatpoint.as_ref().unwrap().0;
                    let new_pos = self.last_beatpoint.as_ref().unwrap().0 + dir * ((rel_time - trans_time_start) / trans_duration) as f32;
